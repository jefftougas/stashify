...
func (pr StashPullRequest) description(desc string) string {
	if desc == "" {
		return pr.Project.CommitMessage()
	}

	return desc
}

func (pr StashPullRequest) title(title string) string {
	if title == "" {
		return strings.Split(pr.Project.CommitMessage(), "\n")[0]
	}
...
func (pr StashPullRequest) Create(title string, description string) error {
	...
	pr.Description = pr.description(description)
	...
	pr.FromRef.Id = pr.Project.CurrentRef()
